...
//import 'package:jwt_decoder/jwt_decoder.dart'; // Commented out as in your version
...
class ApiService {
  ...
  static Future<List<Waste>> getWasteItems() async {
    ...
    print('Raw response from /waste-items: ${response.body}');
    if (response.statusCode == 200) {
      final List<dynamic> data = json.decode(response.body);
      print('Parsed waste items: $data');
  ...
  // Get waste hotspots (new method)
  static Future<List<Map<String, dynamic>>> predictHotspots(String district) async {
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('token');
    if (token == null) throw Exception('No token found');

    final response = await http.post(
      Uri.parse('$baseUrl/predict-hotspots'),
      headers: {
    ...
    print('Predict hotspots response: ${response.body}');
    if (response.statusCode == 200) {
      final List<dynamic> data = json.decode(response.body);
      return data.cast<Map<String, dynamic>>();
    } else {
      throw Exception('Failed to predict hotspots: ${response.body}');
    }
  }
}